...
class JsnModelList extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		if($params->get('search_noempty',0) && $cleanQuery== (string) $query && $app->input->get('search',0) && $this->filter && !$params->get('search_showuser',0)) {
			...
			$app->redirect(JRoute::_('index.php?option=com_jsn&view=list&Itemid='.$app->input->get('Itemid',0),false));
			return null;
		}
		...
		if($service)
			$query->where('b.'.$db->quoteName('prices').' LIKE '.$db->quote('%'.$service.':%'));

		$tag = (int)$app->input->get('tag', 0);

		if($tag)
			$query->where('b.'.$db->quoteName('prices').' LIKE '.$db->quote('%'.$tag.']%'));
		...
		if($app->input->get('layout')=='table' || (strpos(JUri::current(), 'zatochka-remont')) !== false)
