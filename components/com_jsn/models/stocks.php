...
class JsnModelStocks extends JModelList
{
	...
	protected function populateState($stocking = 'name', $direction = 'ASC')
	{
		...
		$stockCol = $app->input->get('filter_order', 'a.name'); //var_dump($this->filter_fields);
		$this->filter_fields=array('a.name');
		if (!in_array($stockCol, $this->filter_fields))
		{
			$stockCol = 'a.name';
		}
		$this->setState('list.stocking', $stockCol);

		$listStock = $app->input->get('filter_order_Dir', 'ASC');
		if (!in_array(strtoupper($listStock), array('ASC', 'DESC', '')))
		{
			$listStock = 'ASC';
		}
		$this->setState('list.direction', $listStock);
	...
	protected function getListQuery()
	{
		...
		if(!(int)$user->id)
			return false;
		$query->select('o.*, o.id AS stock_id, u.*, c.title AS service_name, t.title AS tag_name')->from('#__jsn_stocks AS o');
