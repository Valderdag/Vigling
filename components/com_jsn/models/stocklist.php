...
class JsnModelStocklist extends JModelList
{
	...
	protected function getListQuery()
	{
		...
		if($this->filter)
		{
			...
			if(($app->input->get('limitstart',null)!=null || $app->input->get('start',null)!=null))
			{
		...
		//var_dump($params->get('search_showuser',0));die();
		if($params->get('search_enabled',0) && !$params->get('search_showuser',0) && !$app->input->get('search',0) && $this->filter) return null;
		
		$db = JFactory::getDBO();
		
		$query = $db->getQuery(true);
		$query->select($this->getState('list.select','a.id'))->group('a.id')->from('#__users AS a')->join('left',"#__jsn_users as b ON a.id=b.id")->where('b.stock_prices != ""')->join('left','#__user_usergroup_map as c ON a.id=c.user_id')->where('a.block=0');
		$query->order($db->quoteName('a.id'). 'DESC');
		$cleanQuery=(string) $query;
		if($this->filter) {
			// Load Default Fields
			if($app->input->get('searchid','')!='') $query->where('a.'.$db->quoteName('id').' = '. (int) $app->input->get('searchid',null));
			if($app->input->getString('name')!='')
				$query->where('a.'.$db->quoteName('name').' LIKE '.$db->quote('%'.$app->input->getString('name').'%'));
			if($app->input->get('status','')!='') {
			...
			{
				if($app->input->get($field->alias,'','raw')!=''){
		...
		// Avoid Empty Search
		if($params->get('search_noempty',0) && $cleanQuery== (string) $query && $app->input->get('search',0) && $this->filter && !$params->get('search_showuser',0)) {
			$app->enqueueMessage(JText::_('COM_JSN_NOEMPTYSEARCHWARNING'),'warning');
			$app->redirect(JRoute::_('index.php?option=com_jsn&view=stocklist&Itemid='.$app->input->get('Itemid',0),false));
			return null;
		}
		...
		else
		{
       	...
       	if($app->input->getString('city')!='')
			$query->where('b.'.$db->quoteName('sity').'='.$db->quote($app->input->getString('city')));
		//elseif(class_exists('McsData')){
		//	$city = MCSData::get('cityName');
		//	$query->where('b.'.$db->quoteName('sity').'='.$db->quote($city));
		//}
		
		if($app->input->getString('area')!='')
			$query->where('b.'.$db->quoteName('area').'='.$db->quote($app->input->getString('area')));
			
		if(!empty($h = $app->input->get('home', array(), 'ARRAY'))){
			$where_type = array();
			foreach($h as $type)
				$where_type[] = 'b.'.$db->quoteName('home').' LIKE '.$db->quote('%"'.$type.'"%');
			$query->where('('.implode(' OR ', $where_type).')');
		}
		//var_dump($query->__toString());die();
		$cat_ids = $app->input->get('vyberite_spetsialnos', array(), 'ARRAY');
		if(!empty($cat_ids) && (int)$cat_ids[0])
			$query->where('b.'.$db->quoteName('vyberite_spetsialnos').' LIKE '.$db->quote('%"'.$cat_ids[0].'"%'));
		$service = (int)$app->input->get('service', 0);
		if($service)
			$query->where('b.'.$db->quoteName('stock_prices').' LIKE '.$db->quote('%'.$service.':%'));

		$tag = (int)$app->input->get('tag', 0);
		if($tag)
			$query->where('b.'.$db->quoteName('stock_prices').' LIKE '.$db->quote('%'.$tag.']%'));

		if($app->input->getString('date')!=''){
			$dt = JFactory::getDate($app->input->getString('date'));
			$query->where('b.'.$db->quoteName('work_day').' LIKE '.$db->quote('%'.$dt->format('N').'%'));
		}
		
		if($app->input->get('layout')=='table')
			$query->where("b.is_master='2'");
		else $query->where("b.is_master='1'");
        //var_dump($dt->format('N'));
		//var_dump($query->__toString());die();		
		return $query;
	}
