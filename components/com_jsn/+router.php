...
function JsnBuildRoute(&$query)
{
	...
	if(isset($query['view']))
		switch($query['view'])
		{
			...
			break;
			case 'orders':
				$ordMenu=$menu->getItems('link','index.php?option=com_jsn&view=orders',true);
				if(isset($ordMenu->id))
					$query['Itemid']=$ordMenu->id;
				unset($query['view']);
			break;
			case 'stocks':
				$stockMenu=$menu->getItems('link','index.php?option=com_jsn&view=stocks',true);
				if(isset($stockMenu->id))
					$query['Itemid']=$stockMenu->id;
				unset($query['view']);
			break;
	...
	switch($vars['view'])
	{
		...
		case 'list' :
			if(!empty($segments))
				...
				if((int)$parts[0])
					$vars['vyberite_spetsialnos'][]=(int)$parts[0];
		break;
	}
	return $vars;
}
