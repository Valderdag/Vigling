 ...
 * @since       11.1
 */
class JFormRuleImages extends JFormRule
{
	/**
	...
	public function test(SimpleXMLElement $element, $value, $group = null, JRegistry $input = null, JForm $form = null)
	{
		...
		if(isset($jform['upload_'. (string) $element['name']]))
			$jform_files=$jform['upload_'. (string) $element['name']];
		
		if(count($jform_files))
			for($jform_file = 0; $jform_file < count($jform_files); $jform_file++){ //var_dump($jform_file);die();
				if(isset($jform_file['name']) && strlen($jform_file['name'])>4)
				{
					...
					//$form=JFactory::getApplication()->input->get('jform',null,'raw'); //var_dump($form); var_dump($element['name']); die();
