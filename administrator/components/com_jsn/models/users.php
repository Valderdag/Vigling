...
class JsnModelUsers extends UsersModelUsers
{
	public function __construct($config = array())
	{
		if (empty($config['filter_fields']))
		{
			$config['filter_fields'] = array(
				...
				'activation', 'a.activation', 'sity', 'b.sity', 'is_master', 'b.is_master',
				'active',
				'group_id',
				'range',
				'lastvisitrange',
	...
	protected function getListQuery()
	{
		...
		if($sity = $this->getState('filter.sity'))
			$query->where('b.sity='.$db->quote($sity));
		...
		if($this->filter) {
			...
			foreach($fields as $field)
			{
				if(JFactory::getApplication()->input->get($field->alias,'','raw')!=''){
					...
					//var_dump($this->getState('filter.sity'));
