...
class JsnLinkFieldHelper
{
	...
	public static function loadData($field, $user, &$data)
	{
		...
		if(isset($user->$alias) && $data->$alias=='https://') $data->$alias='';
	}
	...
	public static function storeData($field, $data, &$storeData)
	{
		...
		$alias=$field->alias;
		if(isset($data[$alias])){
			if(substr(trim($data[$alias]), 0, 4)=='https') $storeData[$alias]=$data[$alias];
			else $storeData[$alias]='https://'.$data[$alias];
		}
		if($storeData[$alias]=='https://') $storeData[$alias]='';
	}
