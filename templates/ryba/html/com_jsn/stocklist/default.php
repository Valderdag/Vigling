...
echo(implode(' ',$dispatcher->trigger('renderBeforeStocklist',array($this->items,$this->config))));
...
$doc->addScriptDeclaration("
	...
	function updateSvcStocklist()
	{
		...
		$.ajax({type: 'GET', url: url, dataType: 'html',
            beforeSend: function () { $('.category.jsn_stockList').addClass('waiting') },
            ...
            complete: function () { $('.category.jsn_stockList, .category.jsn_table ' ).removeClass('waiting') },
        });
		return false;
	}
	");
...
<div class="category jsn_stockList">
	<style>
		main>.container{padding: 0;max-width: 1170px;} .category__content-info-list li span{max-width:280px} .category__masters-sidebar .clearable{width:100%} .category__masters-sidebar select{width:100%}
	</style>
	<div class="category__head">
		...
		echo(implode(' ',$dispatcher->trigger('renderBeforeResultStocklist',array($this->items,$this->config))));
		if(/*$this->items && count($this->items)>0 && */ !($this->params->def('search_enabled',0) && !$this->params->def('search_showuser',0) && !JFactory::getApplication()->input->get('search',0)))
	...
	<div class="category__body">
		...
		$this->span=12/$cols;
		$countUsers=0;
		global $JSNSTOCKLIST_DISPLAYED_ID;
		if(is_array($this->items)) foreach($this->items as $item)
		{
			$JSNSTOCKLIST_DISPLAYED_ID=$item->id;
		...
		$JSNSTOCKLIST_DISPLAYED_ID=false;
		?>
		...
		<?php if(!empty($this->items)) : ?>
			...
			<?php endif; ?>
		<?php  endif; ?>
		<?php echo(implode(' ',$dispatcher->trigger('renderAfterResultStocklist',array($this->items,$this->config)))); ?>
		</div> 
		<div class="category__masters-sidebar">
			...
			<form action="<?php echo $flt_url; ?>" class="form-horizontal filter" method="get">
				...
				<input class="submit__search" type="button" value="Поиск" onclick="return updateSvcStocklist()">
...
<?php
echo(implode(' ',$dispatcher->trigger('renderAfterStocklist',array($this->items,$this->config))));
?>
