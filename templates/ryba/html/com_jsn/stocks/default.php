...
<div class="jsn-p">
	...
	if(!$this->items || !count($this->items)):?>
		<h3>Записи на акционные услуги не найдены</h3>
	<?php else:?>
		<h2>Ваши записи на акционные услуги:</h2>
		<?php foreach($this->items as $i=>$stocks){
			$add_style = (JFactory::getDate($stocks->time)<JFactory::getDate())?' text-muted hidden':'';
			if(!$add_style)
				$visible_rows += 1;
			else $hidden_rows += 1;
			$user_m=JsnHelper::getUser($stocks->master_id);
			...
			?>
			<div id="row" class="row m-0<?php echo $add_style ?>" style="padding: 6px 0;">
				...
				<div class="col-sm-7 col-md-6"><span style="font-weight:bold"><?php echo $cat_title;?> - <?php echo $stocks->service_name.($stocks->tag_id ? ' /'.$stocks->tag_name : '');?></span></div>
				...
				<?php //echo $time_sum / 60 .'</br></br>';
                //die(print_r($stocks));
				if(JFactory::getDate($stocks->time)>JFactory::getDate()): ?>
