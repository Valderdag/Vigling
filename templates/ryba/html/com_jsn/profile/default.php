		...
		<div class="accordionWrapper">
			...
			foreach($prices as $id => $prices2) {
				...
				if (!empty($prices2)) {
					...
					$catid = $article->catid;
					$array[$catid]['title'] = $article->category_title;
                    //die(print_r($article->catid));
                    //echo '<div class="accordionItem '.$add_class.'">';
                   // echo '<h2 class="accordionItemHeading'.$counter.'">'.$article->category_title.'</h2>';
                   // echo '<div class="accordionItemContent'.$counter.'"><div class="priceList">';
					...
					foreach($prices2 as $idx=>$price) {
						...
						@$array[$catid]['html'] .= '<div class="priceList__item" data-svc-id="'.$id.'" data-tag-id="'.(int)$tag_id.'">
						<div class="priceList__item-coll price__coll1">'.$array[$catid]['title'].' - '.$name.'</div>
			...
			foreach ($array as $row) {
				$counter++;
				echo '<div class="accordionItem '.$add_class.'">';
				echo '<h2 class="accordionItemHeading'.$counter.'">'.$row['title'].'</h2>';
				echo '<div class="accordionItemContent'.$counter.'"><div class="priceList">';
				echo $row['html'];
				echo '</div></div></div>';
				$add_class = 'closed';
	...
	if(!empty($stock_prices)): ?>
		<div class="container">
			...
			<div class="accordionWrapper">
				...
				if(!empty($stock_prices))
					$counter=0;
				echo '<div style="background-color:#f7cc53; border-radius: 10px; padding: 5px; margin-bottom: 5px" class="accordionItem '.$counter.'"><button style="color:green; font-weight:bold; background-color:#fffff; border-radius: 5px">Акционные услуги:</button>';
				foreach($stock_prices as $id=>$stock_prices2){
					...
					if(!empty($stock_prices2)){
						...
						$arr_stock[$catid]['title'] = $article->category_title;
							...
							foreach($stock_prices2 as $idx=>$stock_price){
								...
								@$arr_stock[$catid]['html'] .= '<div class="stockList__item" data-svc-id="'.$id.'" data-tag-id="'.(int)$tag_id.'">
								<div class="stockList__item-coll stock__coll1">'.$arr_stock[$catid]['title'].' - '.$name.'</div>
					...
					foreach ($arr_stock as $arr_row) {
						$counter++;
						echo '<div class="accordionItem '.$add_class.'">';
						echo '<h2 class="accordionItemHeading'.$counter.'">'.$arr_row['title'].'</h2>';
						echo '<div class="accordionItemContent'.$counter.'"><div class="stockList">';
						echo $arr_row['html'];
						echo '</div></div></div>';
						$add_class = 'closed';
